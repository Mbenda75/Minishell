...
/*   Updated: 2022/03/25 17:49:28 by adaloui          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_decompte	ft_echo_print_single_dollar(char **split_bydollar, t_decompte j_et_s)
{
	char	*str;
...
int	ft_echo_single_dollar(char **cmd, int i)
{
	...
	if (ft_strchr(split_bydollar[j_et_s.l], '?'))
	{
		...
		{
			ft_putchar_fd(split_bydollar[j_et_s.l][j], STDOUT_FILENO);
			j++;
		}
		if (cmd[i + 1])
			write(1, " ", STDOUT_FILENO);
	...
	j_et_s = ft_echo_print_single_dollar(split_bydollar, j_et_s);
	free_str(split_bydollar);
